...
<a class="navbar-brand" style="color:rgb(247, 199, 26); background-color: rgb(22, 53, 175); border-radius: 7px; padding: 0 10px;" href="#">Cross Army Fitness</a>
  ...
  <h1 class="text-center responsive-text" style="-webkit-text-stroke: 2px black; color: transparent; text-shadow: 0px 4px 4px #282828;"> Reporte de Materiales </h1>
  ...
  <a href="inventario crosfit inactivo.php">
...
if(isset($_POST['searchInput'])) {
  ...
  $sql = "SELECT id, nombre, marca, cantidad, peso, estado_del_equipo, precio, imagen, fecha_registro FROM inventario WHERE estatus = 'activo'";
}
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo '<a href="ver inventario.php?id=' . $id . '"class="btn btn-info "><i class="fas fa-eye text-white"></i></a> ';
                    echo '<a href="editar inventario.php?id=' . $id . '"class="btn btn-success "><li class="fas fa-edit text-white"></li></a> ';
                    echo '<a href="cambiar_inactivo_inventario.php?id=' . $id . '"class="btn btn-danger  eliminar-estado" data-id="' . $row['id'] . '"><i class="fas fa-trash-alt text-white"></i></a>';
                    echo '</div>';
                    echo '</div>';
                    echo '</div>';
                    echo '</div>';
              ...
              echo "<div class='alert alert-secondary col-8 text-center text-primary'>No se encontraron materiales.</div>";
              echo "</div>";
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ¿Estás seguro de que deseas eliminar material?
      </div>
    ...
    botonesCambiarEstado.forEach(function(boton) {
        boton.addEventListener('click', function(event) {
            ...
            confirmarCambio.setAttribute('href', 'cambiar_inactivo_inventario.php?id=' + id);
