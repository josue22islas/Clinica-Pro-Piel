...
// Consulta para actualizar el estado a inactivo en membresías vencidas
//$update_sql = "UPDATE membresias SET estatus = 'inactivo' WHERE fecha_termino < '$current_date'";
//$conn->query($update_sql); // Ejecutar la consulta de actualización

//$sql = "SELECT membresias.*, usuarios.nombre as nombre_usuario FROM membresias
 //       INNER JOIN usuarios ON membresias.usuario_id = usuarios.id   WHERE membresias.estatus = 'activo'";
...
$sql_contar_activos = "SELECT COUNT(*) as total_activos FROM membresias WHERE ESTATUS = 'activo'";
$resultado_contar_activos = $conn->query($sql_contar_activos);
...
<div class="container">
    <h1 class="text-center responsive-text" style="-webkit-text-stroke: 2px black; color: transparent; text-shadow: 0px 4px 4px #282828;">Reporte de Membresías</h1>
    <hr>
    ...
    <a href="membresias_vencidas.php">
        <button class="btn btn-danger">Membresías Vencidas</button>
    </a>
    <br><br>
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '<a href="eliminar_membresias.php?id=' . $row["id"] . '"class="btn btn-danger eliminar-estado" data-id="' . $row['id'] . '"><i class="fas fa-trash-alt text-white"></i></a>';//agregamos el id del modal que es igual al id del producto que se pretende eliminar
                        echo '</div>';
                        echo '</td>';
                        echo '</tr>';
                        echo '</tr>';
  ...
  <script>
    ...
    botonesCambiarEstado.forEach(function(boton) {
        boton.addEventListener('click', function(event) {
            ...
            confirmarCambio.setAttribute('href', 'eliminar_membresias.php?id=' + id);
