...
$sql_contar_activos = "SELECT COUNT(*) as total_activos FROM productos_accesorios WHERE ESTATUS = 'inactivo'";
...
if(isset($_POST['searchInput'])) {
    $searchInput = $_POST['searchInput'];
       ...
       $sql = "SELECT id, nombre, marca, cantidad, precio, imagen, fecha_registro FROM productos_accesorios WHERE  ESTATUS = 'inactivo' AND (nombre LIKE '%$searchInput%' OR marca LIKE '%$searchInput%' OR precio LIKE '%$searchInput%')";
} else {
      // Si no se envió el formulario de búsqueda, obtener todos los productos de la consulta...
    $sql = "SELECT id, nombre, marca, cantidad, precio, imagen, fecha_registro FROM productos_accesorios WHERE ESTATUS = 'inactivo'";
}
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<a href="cambiar_activo_accesorios.php?id=' . $id . '"class="btn btn-primary activar-estado" data-id="' . $row['id'] . '">Activar</a> ';
        echo '<a href="eliminar_accesorios.php?id=' . $id . '"class="btn btn-danger  eliminar-estado" data-id="' . $row['id'] . '"><i class="fas fa-trash-alt text-white"></i></a>';
        echo '</td>';
        echo '</tr>';
    }
  ...
  <script>
    ...
    botonesCambiarEstado.forEach(function(boton) {
        boton.addEventListener('click', function(event) {
            ...
            confirmarCambio.setAttribute('href', 'eliminar_accesorios.php?id=' + id);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    botonesCambiarEstado.forEach(function(boton) {
        boton.addEventListener('click', function(event) {
            ...
            confirmarCambio.setAttribute('href', 'cambiar_activo_accesorios.php?id=' + id);
