...
//agregamos un cntador de pendientes 
//$sql_contar_pendiente = "SELECT COUNT(*) as total_pendiente FROM pagos_adeudos WHERE estado = 'pendiente'";
//$resultado_contar_pendiente = $conn->query($sql_contar_pendiente);

//if ($resultado_contar_pendiente->num_rows > 0) {
  //  $row_contar_pendiente = $resultado_contar_pendiente->fetch_assoc();
  //  $total_pendiente = $row_contar_pendiente['total_pendiente'];
//} else {
 //   $total_activos = 0;
//}
...
//$sql = "SELECT pagos_adeudos.*, usuarios.nombre as nombre_usuario FROM pagos_adeudos
       // INNER JOIN usuarios ON pagos_adeudos.usuario_id = usuarios.id WHERE estado = 'pendiente'";//filtro para mostrar los pendientes solamete
...
if(isset($_POST['searchPago'])) {
    ...
    $sql = "SELECT pagos_adeudos.*, usuarios.nombre as nombre_usuario FROM pagos_adeudos
            ...
            WHERE (LOWER(pagos_adeudos.producto) LIKE '%$searchPago%' OR
                   ...
                   LOWER(usuarios.nombre) LIKE '%$searchPago%') AND pagos_adeudos.estado = 'pendiente'";
                   
} else {
    // Consulta predeterminada para mostrar los pendientes si no se realiza una búsqueda
    $sql = "SELECT pagos_adeudos.*, usuarios.nombre as nombre_usuario FROM pagos_adeudos
            INNER JOIN usuarios ON pagos_adeudos.usuario_id = usuarios.id
            WHERE pagos_adeudos.estado = 'pendiente'";
}
...
<div class="container col-sm-11">
    <h1 class="text-center responsive-text" style="-webkit-text-stroke: 2px black; color: transparent; text-shadow: 0px 4px 4px #282828;">Reporte de Pagos y Adeudos</h1>
    <hr>
    ...
    <a href="historial_pagos.php">
        <button class="btn btn-warning">Historial de Pagos</button>
    </a>
    <br><br>
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td style='background-color:rgb(247, 70, 70); color:white;'> $" . $row['deuda'] . "</td>";
                        ...
                        echo "<td><a href='ver_comprobante.php?id={$row['id']}' class='btn btn-info '><i class='fas fa-eye'></i></a> ";
                        echo '<a href="editar pagos y adeudos.php?id=' . $row["id"] . '" class="btn btn-success"><i class="fas fa-edit"></i></a> ';
                        echo "<a href='comprobante_pago.php?id={$row['id']}' class='btn btn-primary'><i class='fas fa-file-word'></i></a></td>";
                       // echo '<a href="estado_pagado.php?id=' . $row["id"] . '"class="btn btn-warning  estado-pagado" data-id="' . $row['id'] . '"><i class="fas fa-dollar-sign" style="color:white;"></i></a>';
                       //echo '<a href="eliminar pagos y adeudos.php?id=' . $row["id"] . '"class="btn btn-warning eliminar-estado" data-id="' . $row['id'] . '">Pagado</a>';//agregamos el id del modal que es igual al id del producto que se pretende eliminar
                        echo '</div>';
                        echo '</td>';
                        echo '</tr>';
                        echo '</tr>';
    ...
    <p>Total de pagos pendientes: <?php echo $total_pendiente; ?></p>
</div><br><br><br><br>
</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Confirmar Estado</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        ¿Estás seguro en realizar los cambios?
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-danger" data-dismiss="modal">Cancelar</button>
        <a href="#" id="confirmareliminacion" class="btn btn-primary">Si, Confirmar</a>
      </div>
    </div>
  </div>
</div>
...
<br><br><br>
  ...
  <script>
    ...
    var botonesCambiarEstado = document.querySelectorAll('.estado-pagado');
    ...
    botonesCambiarEstado.forEach(function(boton) {
        boton.addEventListener('click', function(event) {
            ...
            confirmarCambio.setAttribute('href', 'estado_pagado.php?id=' + id);
