...
<a class="navbar-brand" style="color:rgb(247, 199, 26); background-color: rgb(22, 53, 175); border-radius: 7px; padding: 0 10px;" href="#">Cross Army Fitness</a>
  ...
  <a href="productos admin inactivos.php" style="text-decoration:none;">
...
$sql_contar_activos = "SELECT COUNT(*) as total_activos FROM productos WHERE ESTATUS = 'activo'";
...
if(isset($_POST['searchInput'])) {
    $searchInput = $_POST['searchInput'];
       ...
       $sql = "SELECT id, nombre, marca, cantidad, fecha_registro, imagen, precio FROM productos WHERE ESTATUS = 'activo' AND (nombre LIKE '%$searchInput%' OR marca LIKE '%$searchInput%' OR precio LIKE '%$searchInput%')";
} else {
      // Si no se envió el formulario de búsqueda, obtener todos los productos de la consulta con estatus activo...
    $sql = "SELECT id, nombre, marca, cantidad, imagen, precio, fecha_registro FROM productos WHERE ESTATUS = 'activo'";
}
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<a href="ver producto.php?id=' . $id . '" class="btn btn-info " ><i class="fas fa-eye text-white"></i></a> ';
        echo '<a href="modificar_producto.php?id=' . $id . '"class="btn btn-success "><li class="fas fa-edit text-white"></li></a> ';
        echo '<a href="cambiar_inactivo_producto.php?id=' . $id . '"class="btn btn-danger eliminar-estado" data-id="' . $row['id'] . '"><i class="fas fa-trash-alt text-white"></i></a> ';
        echo '</td>';
        echo '</tr>';
    }
  ...
  <script>
    ...
    botonesCambiarEstado.forEach(function(boton) {
        boton.addEventListener('click', function(event) {
            ...
            confirmarCambio.setAttribute('href', 'cambiar_inactivo_producto.php?id=' + id);
