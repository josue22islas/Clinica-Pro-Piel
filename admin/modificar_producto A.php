...
$dbname = "u990524785_cross_army";
  ...
  $sql = "SELECT id, nombre, estatus, precio, marca, cantidad, imagen, fecha_registro FROM productos_accesorios WHERE id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $sql = "UPDATE productos_accesorios SET nombre=?, precio=?, marca=?, cantidad=?, fecha_registro=? WHERE id=?";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param("sssssi", $nombre, $precio, $marca, $cantidad, $fecha_registro, $id);
  $stmt->execute();
  $stmt->close();
  $conn->close();

  // Redirigir a la página de reporte de atletas después de la actualización
  header('Location: productos admin accesorios.php');
  exit;
}
?>
...
?>
   <button class="btn btn-primary" type="submit">Actualizar</button>
            ...
            <a href="productos admin accesorios.php"><button class="btn btn-success" type="button">Regresar</button></a>
