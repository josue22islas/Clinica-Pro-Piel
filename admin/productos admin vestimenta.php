...
<a class="navbar-brand" style="color:rgb(247, 199, 26); background-color: rgb(22, 53, 175); border-radius: 7px; padding: 0 10px;" href="#">Cross Army Fitness</a>
        ...
        <div class="collapse navbar-collapse" id="navbarNavDropdown">
          ...
          <ul class="navbar-nav">
            ...
            <li class="nav-item">
              <a class="nav-link" href="productos admin vestimenta.php"><i class="fas fa-cart-plus" style="color: #ffffff;"></i></i>/Productos</a>
            </li>
            <li class="nav-item">
     ...
     <div class="container col-sm-11" style="background-color: rgba(255, 255, 255, 0.9);  border-radius:10px;"><br>      
      ...
      <div class="container col-sm-10">
        <div class="title-cards">
            <h1 class="text-center responsive-text" style="-webkit-text-stroke: 2px black; color: transparent; text-shadow: 0px 4px 4px #282828;">Reporte de Productos</h1>
            <hr>
        </div>
    ...
    <button class="btn btn-dark" onclick="redireccionar()">Ir a la categoría</button>
        ...
        <a href="registrar vestimenta.php" style="text-decoration:none;">
    <button class="btn btn-primary">Registrar Productos</button>
  </a>

  <a href="productos vestimenta inactivos.php" style="text-decoration:none;">
    <button class="btn btn-danger my-2">Inactivos</button>
  </a>

  <a href="ver vestimenta.php">
...
$sql_contar_activos = "SELECT COUNT(*) as total_activos FROM productos_vestimenta WHERE ESTATUS = 'activo'";
...
if(isset($_POST['searchInput'])) {
    $searchInput = $_POST['searchInput'];
       ...
       $sql = "SELECT id, nombre, marca, cantidad, talla, fecha_registro, imagen, precio FROM productos_vestimenta WHERE ESTATUS = 'activo' AND (nombre LIKE '%$searchInput%' OR marca LIKE '%$searchInput%' OR precio LIKE '%$searchInput%')";
} else {
      // Si no se envió el formulario de búsqueda, obtener todos los productos de la consulta...
    $sql = "SELECT id, nombre, marca, cantidad, talla, precio, imagen, fecha_registro FROM productos_vestimenta WHERE ESTATUS = 'activo'";
}
...
if ($result->num_rows > 0) {
    // Mostrar los productos en una tabla responsiva
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<td style="padding: 30px;">' . $talla . '</td>';
        echo '<td style="padding: 30px;">$' . number_format($precio, 2) . ' MXN</td>';
        
        echo '<td style="padding: 20px;"  >';
        echo '<a href="ver producto V.php?id=' . $id . '" class="btn btn-info"><i class="fas fa-eye text-white"></i></a> ';
        echo '<a href="modificar_producto V.php?id=' . $id . '"class="btn btn-success"><li class="fas fa-edit text-white"></li></a> ';
        echo '<a href="cambiar_inactivo_vestimenta.php?id=' . $id . '"class="btn btn-danger eliminar-estado" data-id="' . $row['id'] . '"><i class="fas fa-trash-alt text-white"></i></a>';
        echo '</td>';
        echo '</tr>';
    }
...
<div class="col-sm-12 text-center">
    <p>Total de productos activos: <?php echo $total_activos; ?></p>
</div><br><br><br><br>
  </div>
  </div>
  ...
  <script>
    ...
    botonesCambiarEstado.forEach(function(boton) {
        boton.addEventListener('click', function(event) {
            ...
            confirmarCambio.setAttribute('href', 'cambiar_inactivo_vestimenta.php?id=' + id);
