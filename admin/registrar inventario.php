...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $precio = $_POST["precio"];
    $estado_del_equipo = $_POST["estado_del_equipo"];
    ...
    if (!empty($_FILES["imagen"]["tmp_name"]) && file_exists($_FILES["imagen"]["tmp_name"])) {
        ...
        if ($_FILES["imagen"]["size"] > $maxSize) {
            $mensaje = "<div class='alert alert-danger'>El tamaño de la imagen es demasiado grande. Por favor, sube una imagen de máximo 5MB.</div>";
        } else {
            ...
            if (in_array(strtolower($extension), $extensionesPermitidas)) {
              ...
              $dbname = "u990524785_cross_army";
                ...
                $sql = "INSERT INTO inventario (nombre, marca, cantidad, peso, estado_del_equipo, precio, imagen, fecha_registro) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sssssdss", $nombre, $marca, $cantidad, $peso, $estado_del_equipo, $precio, $imagen, $fecha_registro);

                if ($stmt->execute()) {
                    $mensaje = "<div class='alert alert-success'>Material registrado correctamente.</div>";
                } else {
                    $mensaje = "<div class='alert alert-danger'>Error al registrar el material: " . $stmt->error . "</div>";
                }
        ...
        $mensaje = "<div class='alert alert-danger'>No se ha enviado el formulario.</div>";
    }
}
?>
...
.navbar-logo {
  ...
  @media only screen and (max-width: 600px) {
    font-size: 2.2em; /* Tamaño del texto para pantallas más pequeñas */
  }
      ...
      <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                <h1 class="text-center responsive-text" style="-webkit-text-stroke: 2px black; color: transparent; text-shadow: 0px 4px 4px #282828;">Registrar Material</h1><hr>

            
                    <div class="form-group">
                        <label for="nombre">Nombre del material:</label>
                        <input type="text" id="nombre" name="nombre" placeholder="Ingrese nombre del material" class="form-control" required>
                    </div>

                    <div class="form-group">
                        <label for="marca">Marca:</label>
                        <input type="text" id="marca" name="marca" placeholder="Ingrese marca del material" class="form-control" required>
                    </div>

                    <div class="form-group">
                    <label for="cantidad">Cantidad:</label>
            <select class="form-control" id="cantidad" name="cantidad" required>
              <option value="" disabled selected>Ingrese cantidad del material . . .</option>
                ...
                <option value="4 Unidades">4 Unidades</option>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" id="peso" name="peso" placeholder="Agrega un peso Kg" class="form-control" required>
                    </div>

                    <div class="form-group">
                        <label for="precio">Precio:</label>
                        <input type="number" step="0.01" id="precio" name="precio" placeholder="Agrega un precio $" class="form-control" required>
                    </div>

                    <div class="form-group">
                    <label for="estado_del_equipo">Estado del equipo:</label>
            <select class="form-control" id="estado_del_equipo" name="estado_del_equipo" required>
              <option value="" disabled selected>Ingrese estado del equipo . . .</option>
                ...
                <option value="Malo">Malo</option>
                    ...
                    <div class="form-group">
                        <label for="imagen" class="text-primary">Imagen del material (Solo formatos PNG, JPEG):</label>
                        <input type="file" id="imagen" name="imagen" accept="image/*" class="form-control-file" required>
                    </div>

                    <button type="submit" class="btn btn-primary">Registrar</button>
                    <a class="btn btn-danger" href="Inventario crosfit.php">Cancelar</a>
                </form><br>
