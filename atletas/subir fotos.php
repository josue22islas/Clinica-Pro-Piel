...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($_FILES["imagen"]["tmp_name"]) && file_exists($_FILES["imagen"]["tmp_name"])) {
        ...
        if (!in_array($tipoImagen, $allowedTypes)) {
            header("location: galeria de usuario.php?error=Solo+se+permite+imágenes+en+formato+PNG+y+JPG");//mandar el mensaje por el metodo get 
            exit(); // Asegúrate de salir después de redirigir
           // $mensaje = "Solo se permiten imágenes en formato PNG y JPG.";
        } elseif ($tamanioImagen > 5242880) { // 5MB en bytes
            header("location: galeria de usuario.php?error=La+imagen+es+demasiado+grande.+El+tamaño+máximo+permitido+es+5MB.");//mandar el mensaje por el metodo get 
            exit(); // Asegúrate de salir después de redirigir
